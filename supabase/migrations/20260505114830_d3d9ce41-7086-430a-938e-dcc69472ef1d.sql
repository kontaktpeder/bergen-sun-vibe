create or replace function public.submit_contribution(_type text, _venue_id uuid, _data jsonb, _is_confirm boolean default false, _new_venue jsonb default null)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if _type = 'venue_add' then
    ...
    if _venue is null then raise exception 'venue_id required'; end if;
    ...
    if _type = 'beer_price' then
      ...
      if (_data->>'status') not in ('sun','shade') then raise exception 'invalid sun status'; end if;
    elsif _type = 'photo' then
      if coalesce(_data->>'image_url','') = '' then raise exception 'image_url required'; end if;
    end if;
  ...
  update public.profiles set points = coalesce(points, 0) + _award
    where id = _user returning points into _new_points;
