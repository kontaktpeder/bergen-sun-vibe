-- 1) Ensure city column + check constraint + index
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'venues_city_check'
  ) then
    alter table public.venues
      add constraint venues_city_check
      check (city is null or city in ('Bergen', 'Oslo'));
  end if;
end $$;

create index if not exists venues_city_idx on public.venues(city);

-- 2) Backfill from coordinates
update public.venues
set city = 'Bergen'
where city is null
  and lat between 60.34 and 60.44
  and lng between 5.20 and 5.45;

update public.venues
set city = 'Oslo'
where city is null
  and lat between 59.82 and 59.99
  and lng between 10.60 and 10.95;

-- 3) Patch submit_contribution to require + store city on venue_add
...
begin
  ...
  if _type = 'venue_add' then
    ...
    insert into public.venues (name, category, lat, lng, image_url, slug, tags, last_activity_at, city)
    values (_new_name, _new_venue->>'category', _new_lat, _new_lng, nullif(_new_venue->>'image_url',''), _slug, '{}', now(), _new_city)
