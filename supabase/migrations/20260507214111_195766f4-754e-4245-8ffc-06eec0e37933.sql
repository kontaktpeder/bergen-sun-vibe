...
declare
  ...
  _close_lat numeric := 0.0027;
  _close_lng numeric;
  _far_lat numeric := 0.018;
  _far_lng numeric;
begin
  if _user is null then
    raise exception 'not authenticated';
  end if;
  if _type not in ('sun_report','beer_price','photo','venue_add','crowd_report') then
    raise exception 'invalid contribution type';
  end if;

  _award := case _type
    when 'sun_report' then 5
    when 'crowd_report' then 5
  ...
  if _type = 'venue_add' then
    ...
    if _type = 'beer_price' then
      ...
      if (_data->>'status') not in ('sun','partial','shade','going_down') then
        raise exception 'invalid sun status';
      end if;
    elsif _type = 'crowd_report' then
      if (_data->>'level') not in ('quiet','some','full','queue') then
        raise exception 'invalid crowd level';
      end if;
  ...
  if _type = 'sun_report' then
    _sun_status := case
      when _data->>'status' = 'sun' then 'sun-now'
      when _data->>'status' = 'partial' then 'sun-now'
      when _data->>'status' = 'going_down' then 'shade-soon'
      else 'shade'
    end;
